-- ============================================================================
-- prisma/ddl/schema.template.sql — getemplatiseerde DDL (Fase 1, bevroren
-- contract). Eén bron waaruit ELK client-schema wordt aangemaakt
-- (bv. `bts`, `clientb`) door de `{{schema}}`-placeholder te vervangen.
--
...
--
-- Vorm gespiegeld naar prisma/schema.prisma — zie dat bestand voor de volledige
...
--
-- Substitutie (voorbeeld, PowerShell — pas {{schema}} toe vóór uitvoeren):
...
-- ============================================================
-- 0. Schema zelf (idempotent aanmaken — zie ook de per-client
--    onboarding-runbook in het plan, §8, die dit als los stap benoemt)
-- ============================================================
...
-- 1. Users
-- ============================================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE schema_id = SCHEMA_ID('{{schema}}') AND name = 'users')
...
-- ============================================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE schema_id = SCHEMA_ID('{{schema}}') AND name = 'app_config')
...
-- ============================================================
-- 3. Import Jobs (queue-based herontwerp — zie kop-comment)
-- ============================================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE schema_id = SCHEMA_ID('{{schema}}') AND name = 'import_jobs')
...
-- ============================================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE schema_id = SCHEMA_ID('{{schema}}') AND name = 'audit_log')
...
-- ============================================================
-- 5. VAT Return Entries — Audit trail for booked correction entries
-- ============================================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE schema_id = SCHEMA_ID('{{schema}}') AND name = 'vat_return_entries')
CREATE TABLE {{schema}}.vat_return_entries (
    id              INT IDENTITY(1,1) PRIMARY KEY,
    company_id      INT             NOT NULL,
...
CREATE NONCLUSTERED INDEX ix_vat_return_entries_company_id
    ON {{schema}}.vat_return_entries (company_id);
GO

-- Filtered unique index (allows multiple dismissed rows per company/period;
-- NOT representable in prisma/schema.prisma — see the note there).
...
CREATE UNIQUE NONCLUSTERED INDEX uq_vat_return_active_company_period
    ON {{schema}}.vat_return_entries (company_id, period)
    WHERE dismissed = 0;
GO
...
-- 6. CSV Blob Sync Log
-- ============================================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE schema_id = SCHEMA_ID('{{schema}}') AND name = 'csv_blob_sync_log')
CREATE TABLE {{schema}}.csv_blob_sync_log (
    id              INT IDENTITY(1,1) PRIMARY KEY,
...
CREATE NONCLUSTERED INDEX ix_csv_blob_sync_log_blob_name
    ON {{schema}}.csv_blob_sync_log (blob_name, started_at DESC);
GO
...
CREATE NONCLUSTERED INDEX ix_csv_blob_sync_log_status
    ON {{schema}}.csv_blob_sync_log (status, started_at DESC);
GO

-- ============================================================
-- 7. User <-> Company assignments (multi-tenant access control)
-- ============================================================
IF NOT EXISTS (SELECT * FROM sys.tables WHERE schema_id = SCHEMA_ID('{{schema}}') AND name = 'user_companies')
...
CREATE NONCLUSTERED INDEX ix_user_companies_company
    ON {{schema}}.user_companies (company_id);
GO
